...
while error = 1
	...
	if y = "" then
		...
		fl.Close
		msgbox "Você tem um NIC (cartão de interface de rede) com drivers que não estão incluídos no Windows XP. Instale os drivers agora e, em seguida, clique em OK para continuar a instalação automática."
	else
		Error = 0
	end if
