...
strProductName = "Console de gerenciamento de diretivas de grupo"
...
strMsgError = "A instalação não pode copiar os arquivos do " & strProductName & ". Depois que a instalação automática for finalizada, " & chr(10) & _
		      "termine a instalação do computador do instrutor seguindo as instruções de " & chr(10) & _
	    	  "instalação do " & strProductName & " no guia de instalação automática."
...
if ServerPath <> False then
	for each pathfromroot in pathset
		...
		if fso.FileExists(lookfor) then 
			...
			' msgbox "O arquivo não foi encontrado ou o caminho de rede não está disponível."
		end if
	next
end if
