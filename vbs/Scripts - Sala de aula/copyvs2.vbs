v' This script copies the product installation files from a CD, Product Server, or
...
strProductName = "Visual Studio .NET disc 1"
...
sub copyfiles(path)
	...
	if not(fso.FolderExists("C:\VSNet")) then fso.CreateFolder "C:\VSNet"
	wshshell.Run "net share VSNet=C:\VSNet"
	on error resume next
		'copy setup folder
		sSrc = path & "\*.*"
		sDest = "C:\VSNet" 
