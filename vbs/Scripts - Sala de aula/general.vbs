	...
	For each d in dc
		...
		if dl.isready then
			if (fso.FileExists(lookfor)) then
				Set ts = fso.CreateTextFile("c:\tools\copyftp.vbs", True)
				ts.WriteLine "Set oSHApp = CreateObject(" & chr(34) & "Shell.Application" & chr(34) & ")" 
				ts.writeline "sSrc = " & chr(34) & dl & "\FTProot\*.*" & chr(34)
				ts.writeline "sDest = " & chr(34) & "c:\inetpub\ftproot" & chr(34)
				ts.writeline "oSHApp.Namespace(sDest).CopyHere sSrc, &H10&"
				ts.writeline "WScript.Quit"
				ts.Close
				WshShell.run "c:\tools\copyftp.vbs", 1, true
				wscript.quit
			end if
		end If
	Next
