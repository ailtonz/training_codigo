...
'					INSTRUÇÕES DO LAB
'
' 1º Crie logo abaixo a VARIÁVEL txt e a MATRIZ servidores
' 2º Defina os valores da MATRIZ com o nome do seu servidor e o servidor do seu parceiro
' 3º Substitua os textos em colchetes com as funções de dimensão de matriz (lbound(matriz) 
...
for x = lbound(servidores) to ubound(servidores)
	...
	For Each Drive In coldisk
		if not isnull (drive.size) then
			txt = txt & " Drive " & Drive.deviceid & " contém " _
